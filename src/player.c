...
/*   Updated: 2025/09/17 00:46:02 by thasousa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_game *g, char dir)
{
	...
	nx = g->player->x + dx;
	ny = g->player->y + dy;
	if (is_blocked(g, nx, ny) || !handle_tile(g, nx, ny))
		return ;
	old_x = g->player->x;
	old_y = g->player->y;
	g->player->x = nx;
	...
	g->moves++;
	print_moves(g);
	update_player_pos(g, old_x, old_y);
}
...
void	game_loop(t_game *game)
{
	game->mlx = mlx_init();
	if (!game->mlx)
		ft_error("Falha ao iniciar mlx");
	game->win = mlx_new_window(game->mlx,
			TILE_SIZE * game->cols,
			TILE_SIZE * game->lines,
			"So Long");
	if (!game->win)
		ft_error("Falha ao criar janela");
	mlx_hook(game->win, 17, 0, exit_game, game);
	load_images(game);
	redraw_map(game);
	mlx_key_hook(game->win, handle_keypress, game);

	// Aqui: registra loop_handler para rodar todo frame
...
void	update_player_pos(t_game *g, int old_x, int old_y)
{
	...
	if (g->map[g->player->y][g->player->x] == 'E')
		mlx_put_image_to_window(g->mlx, g->win,
			g->img_player_on_exit,
			g->player->x * TILE_SIZE,
			g->player->y * TILE_SIZE);
	else
		mlx_put_image_to_window(g->mlx, g->win,
			g->img_player,
			g->player->x * TILE_SIZE,
